...
const expectRow = (berlinClockString, rowIndex, expectedRow) =>
  expect(berlinClockRow(berlinClockString, rowIndex)).toEqual(expectedRow);

describe(
  "The 'Berlin Clock' is the first public clock in the world that tells the time by means of illuminated, colored fields," +
  ...
  () => {
    ...
    describe('The top and second rows displaying the hour value in 24-hour format.', () => {
      ...
      const expectTopRow = (berlinClockString, expectedLights) =>
        expectRow(berlinClockString, 1, expectedLights);
      const expectSecondRow = (berlinClockString, expectedLights) =>
        expectRow(berlinClockString, 2, expectedLights);
    });
    ...
    describe('The third and bottom rows displaying the minutes value.', () => {
      ...
      const expectBottomRow = (berlinClockString, expectedLights) =>
        expectRow(berlinClockString, 4, expectedLights);
      const expectThirdRow = (berlinClockString, expectedLights) =>
        expectRow(berlinClockString, 3, expectedLights);
    });

    describe('The time in input is specified and validated as a 24H format.', () => {
      it("Expect that the TIME is between '00:00:00' and '23:59:59'", () => {
